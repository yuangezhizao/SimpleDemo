...
namespace GuardSystem
{
    public class CompressServer
    {
        ...
        /// 将程序压缩并打包
        /// </summary>
        public static void PackServerPluse1()
        {
            if (!Directory.Exists(SystemConfig.PackFilePath) || string.IsNullOrEmpty(SystemConfig.ToGuardProcessName))
                return;
            ...
            foreach (var file in list)
            {
                if (Regex.IsMatch(file, SystemConfig.PackExcludeFilesRex, RegexOptions.Singleline))
                    continue;
                content.Append(file.Replace(SystemConfig.PackFilePath, ""));
                content.Append(" 11111111111111100000000000000000000 ");
                content.Append(File.ReadAllText(file));
                content.Append(" 00000000000000000000111111111111111 ");
