...
namespace BLL.Sprider.classInfo
{
    ...
    /// 也买酒分类处理
    /// </summary>
    public class YesmywineClassInfo : Yesmywine, ISiteClassBLL
    {
        public YesmywineClassInfo()
        {
            Baseinfo = new SiteInfoDB().SiteById(135);
        }
        ...
        public void SaveAllSiteClass()
        {
            ...
            string directoryHtml = HtmlAnalysis.Gethtmlcode("http://list.yesmywine.com/");
        ...
        {
            string catArea = RegGroupsX<string>(directoryHtml,"<h4>所有分类</h4>(?<x>.*?)<div class=\"channel-topboard\">");
            if (catArea == null)
                return;

            var list = RegGroupCollection(catArea, "<a rel=\"nofollow\" href='(?<x>.*?)'> (?<y>.*?)</a>|<a href=\"(?<x>.*?)\" >(?<y>.*?)</a>");

            foreach (Match item in list)
            {
                string tempUrl = item.Groups["x"].Value;
                string tempName = item.Groups["y"].Value;
                string tempid = RegGroupsX<string>(tempUrl, "/(?<x>.*?)$");
                if (!HasBindClasslist.Exists(c => c.ClassId == tempid))
                {
                    SiteClassInfo iteminfo = new SiteClassInfo
                    {
                        ...
                        Urlinfo = string.Format("http://list.yesmywine.com/{0}", tempid),
